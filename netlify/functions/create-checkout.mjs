...
import { cleanText, getSiteUrl, getSupabaseAdmin, json, roundMoney } from './lib.mjs';
...
export default async function handler(request) {
  ...
  try {
    ...
    if (!name || !email || !phone || !address || !neighborhood || !items.length) {
      return json({ error: 'Preencha nome, e-mail, WhatsApp, endereço, bairro e o carrinho.' }, 400);
    }
    if (!/^\S+@\S+\.\S+$/.test(email)) return json({ error: 'Informe um e-mail válido.' }, 400);
    if (items.length > 30) return json({ error: 'O carrinho contém itens demais.' }, 400);

    const supabase = getSupabaseAdmin();
    const productIds = [...new Set(items.map(item => cleanText(item.productId, 60)).filter(Boolean))];
    ...
    if (!products || products.length !== productIds.length) {
      return json({ error: 'Um dos produtos não está mais disponível. Atualize a página.' }, 409);
    }
    ...
    for (const rawItem of items) {
      ...
      if (!product || !product.sizes?.includes(size)) {
        return json({ error: 'Produto ou tamanho inválido no carrinho.' }, 400);
      }
      if (!Number.isInteger(quantity) || quantity < 1 || quantity > 20) {
        return json({ error: 'Quantidade inválida no carrinho.' }, 400);
      }

      const available = Number(product.stock_by_size?.[size] || 0);
      if (quantity > available) {
        return json({ error: `Estoque insuficiente para ${product.name}, tamanho ${size}.` }, 409);
      }
    ...
    const total = roundMoney(subtotal + DELIVERY_FEE);
    ...
    const preferenceBody = {
      ...
      payer: { name, email },
      shipments: { cost: DELIVERY_FEE, mode: 'not_specified' },
      external_reference: order.id,
      metadata: { order_id: order.id, order_number: String(order.order_number) },
      statement_descriptor: 'VITTA FIT WEAR',
    ...
    return json({ error: 'Não foi possível iniciar o pagamento. Confira as configurações e tente novamente.' }, 500);
  }
}
