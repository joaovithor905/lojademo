...
async function loadProducts() {
  ...
  const { data, error } = await supabase.from('products').select('*').eq('active', true).order('featured', { ascending: false }).order('created_at', { ascending: false });
...
function openProduct(productId) {
  ...
  qs('#productModalThumbnails').innerHTML = images.map((image, index) => `<button class="product-thumb ${index === 0 ? 'active' : ''}" type="button" onclick="selectProductImage(${index})"><img src="${escapeHtml(image)}" alt="Foto ${index + 1} de ${escapeHtml(product.name)}"></button>`).join('');
  const sizeSelect = qs('#productModalSize');
  sizeSelect.disabled = !availableSizes.length;
  sizeSelect.innerHTML = availableSizes.length ? availableSizes.map(size => `<option value="${escapeHtml(size)}">${escapeHtml(size)} — ${stockFor(product, size)} disponível(is)</option>`).join('') : '<option>Esgotado</option>';
...
function cartDetailed() { return cart.map(item => ({ ...item, product: products.find(product => product.id === item.productId) })).filter(item => item.product); }
function renderCart() {
  ...
  qs('#cartItems').innerHTML = detailed.map(item => `<div class="cart-item"><img src="${escapeHtml(productImages(item.product)[0])}" alt="${escapeHtml(item.product.name)}"><div><h4>${escapeHtml(item.product.name)}</h4><small>Tamanho ${escapeHtml(item.size)} · ${money(item.product.price)}</small><div class="qty-row"><button onclick="changeQty('${item.productId}','${escapeHtml(item.size)}',-1)">−</button><strong>${item.quantity}</strong><button onclick="changeQty('${item.productId}','${escapeHtml(item.size)}',1)">+</button></div></div><button class="remove-item" onclick="removeItem('${item.productId}','${escapeHtml(item.size)}')">×</button></div>`).join('');
...
function changeQty(productId, size, delta) {
  ...
  item.quantity += delta;
  if (item.quantity <= 0) removeItem(productId, size); else persistCart();
}
function removeItem(productId, size) { cart = cart.filter(item => !(item.productId === productId && item.size === size)); persistCart(); }
function openCart() { qs('#cartDrawer').classList.add('open'); qs('#cartBackdrop').classList.remove('hidden'); }
function closeCart() { qs('#cartDrawer').classList.remove('open'); qs('#cartBackdrop').classList.add('hidden'); }
function showToast(message) { const element = qs('#toast'); element.textContent = message; element.classList.add('show'); setTimeout(() => element.classList.remove('show'), 3200); }
...
qs('#checkoutForm').addEventListener('submit', async event => {
  ...
  const payload = { customer: { name: form.get('name'), email: form.get('email'), phone: form.get('phone'), address: form.get('address'), neighborhood: form.get('neighborhood') }, notes: form.get('notes'), items: cart.map(item => ({ productId: item.productId, size: item.size, quantity: item.quantity })) };
  try {
    ...
    localStorage.setItem('vitta-pending-order', JSON.stringify({ orderId: result.orderId, token: result.publicToken, orderNumber: result.orderNumber }));
...
qs('#categoryFilters').addEventListener('click', event => { if (!event.target.matches('.filter')) return; document.querySelectorAll('.filter').forEach(button => button.classList.remove('active')); event.target.classList.add('active'); currentCategory = event.target.dataset.category; renderProducts(); });
...
renderCart();
loadProducts().catch(error => { console.error(error); showToast('Não foi possível carregar os produtos.'); });
