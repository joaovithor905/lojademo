...
const paymentLabels = {
  ...
  approved: 'Aprovado',
  approved_stock_issue: 'Aprovado — loja revisando estoque',
  rejected: 'Recusado',
  cancelled: 'Cancelado',
...
function updateVisual(data) {
  ...
  qs('#paymentStatus').textContent = paymentLabels[data.payment_status] || data.payment_status;

  const icon = qs('#statusIcon');
  if (['approved', 'approved_stock_issue'].includes(data.payment_status)) {
  ...
  if (['rejected', 'cancelled', 'error'].includes(data.payment_status) || result === 'failure') {
    ...
    icon.textContent = '×';
    qs('#statusTitle').textContent = 'Pagamento não concluído';
  ...
  qs('#statusMessage').textContent = 'A confirmação ainda está sendo processada pelo Mercado Pago. Esta página será atualizada automaticamente.';
  return false;
}

async function checkStatus() {
  if (!order || !token) {
    qs('#statusTitle').textContent = 'Pedido não identificado';
...
(async () => {
  try {
    ...
    qs('#statusTitle').textContent = 'Não foi possível consultar';
    qs('#statusMessage').textContent = error.message;
  }
})();
